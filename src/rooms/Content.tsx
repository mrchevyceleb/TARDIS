...
export function DraftDesk({ initial, status, onBack, onSaved,packageMode=false,registerLeave }: { initial: ContentDraft; status?: ContentStatus; onBack: () => void; onSaved: () => void; packageMode?:boolean;registerLeave?:(fn:(()=>Promise<void>)|null)=>void }) {
  ...
  return <section className="content-room content-desk">
    ...
    <div className="content-edit-bar"><div className="content-segment"><button aria-pressed={!preview} onClick={() => setPreview(false)}>Edit source</button><button aria-pressed={preview} onClick={() => setPreview(true)}>Preview</button></div><button className="content-button" disabled={readOnly || !!busy} onClick={() => setReviseOpen(true)}><Sparkles size={16}/> Ask for a change</button></div>
    {draft.kind==='video'?<div className="content-paper"><video className="package-video" controls preload="metadata" src={safeUrl(draft.payload.video_url)}/><p className="content-hint">Use ?Ask for a change? to revise captions or visuals. Caption-only changes reuse the existing clips.</p>{draft.payload.beats?.map(beat=><article className="content-social-preview" key={beat.beat}><strong>Scene {beat.beat}</strong><p>{beat.on_screen_text}</p><details><summary>Visual direction</summary>{beat.visual_prompt}</details></article>)}</div>:<div className="content-paper">
