...
function CreateContent({ brand, onClose, onCreated }: { brand: ContentBrand; onClose: () => void; onCreated: () => void }) {
  ...
  const submit = async () => {
    ...
    try { await contentRequest('/generate', 'POST', { brand, brief, kinds, engine, requestId: request.current.id }); onCreated(); }
    catch (e) { setError(messageOf(e)); } finally { setBusy(false); }
  };
  return <ContentModal title={`Create for ${CONTENT_BRANDS[brand]}`} onClose={busy ? undefined : onClose}><form onSubmit={(e) => { e.preventDefault(); void submit(); }}><label className="content-field">What would you like to say?<textarea autoFocus required maxLength={12000} rows={5} placeholder="A helpful post about… Include your angle, audience, or a link to work from." value={brief} onChange={(e) => setBrief(e.target.value)} /></label><fieldset className="content-choices"><legend>Make it into</legend>{[['blog','A blog post'],['social-pack','Social posts']].map(([id,name]) => <label key={id}><input type="checkbox" checked={kinds.includes(id)} onChange={(e) => setKinds((old) => e.target.checked ? [...old,id] : old.filter((kind) => kind !== id))}/>{name}</label>)}</fieldset><EngineSelect value={engine} onChange={setEngine}/><p className="content-hint">Your brand voice is included automatically. Everything starts as a draft.</p>{error && <p className="content-notice error" role="alert">{error}</p>}<button className="content-button primary" type="submit" disabled={busy || brief.trim().length < 5 || !kinds.length}>{busy ? <LoaderCircle className="content-spin" size={17}/> : <Sparkles size={17}/>} {busy ? 'Starting your drafts…' : 'Create drafts'}</button></form></ContentModal>;
}
