...
import { missingContentImages, CONTENT_BRANDS, CONTENT_CHANNELS, CONTENT_ENGINES, type ContentImage, type ContentBrand, type ContentChannel, type ContentDraft, type ContentEngine, type ContentJob, type ContentStatus } from '../data/content';
import './content.css';
...
import { ContentSocialStudio } from './ContentSocialStudio';
import { ContentVideoStudio } from './ContentVideoStudio';
...
const label = (value: string) => value === 'saved-draft' ? 'Saved in GHL' : value === 'in-review' || value === 'draft' ? 'Needs review' : value === 'in-progress' ? 'Writing' : value.charAt(0).toUpperCase() + value.slice(1);
...
export function Content() {
  ...
  const [brand, setBrand] = useState<ContentBrand>(()=>(Object.hasOwn(CONTENT_BRANDS,new URLSearchParams(location.search).get('brand')??'')?new URLSearchParams(location.search).get('brand'):'operly') as ContentBrand);
...
export function DraftDesk({ initial, status, onBack, onSaved,packageMode=false,registerLeave }: { initial: ContentDraft; status?: ContentStatus; onBack: () => void; onSaved: () => void; packageMode?:boolean;registerLeave?:(fn:(()=>Promise<void>)|null)=>void }) {
  ...
  const [targetPost,setTargetPost]=useState<number|undefined>();
  const [engine, setEngine] = useState<ContentEngine>('claude');
  ...
  const readOnly = !!draft.package_plan_id || ['published','spiked','in-progress'].includes(draft.status) || !!draft.publications?.some((item) => !!item.provider_id || ['sending','scheduled','published','unknown','saved-draft'].includes(item.status));
  ...
  const scheduling = draft.kind!=='email' && selectedChannels.length > 0 && selectedChannels.every((channel) => connections.some((item) => item.channel === channel && item.schedulingSupported));
  ...
  const flush = useCallback(async (): Promise<void> => {
    ...
    const work = (async () => {
      const patch = {
        ...
        ...(initial.kind === 'email' ? { payload: { subject: captured.payload.subject, preview_text: captured.payload.preview_text, alt_subjects: captured.payload.alt_subjects, image: captured.payload.image } } : {}),
      };
  ...
  useEffect(()=>{registerLeave?.(flush);return()=>registerLeave?.(null);},[registerLeave,flush]);
  ...
  return <section className="content-room content-desk">
    <header className="content-desk-header">{packageMode ? <span className={`content-badge ${dirty ? 'draft' : draft.status}`}>{dirty ? 'Needs review' : label(draft.status)}</span> : <button className="content-button quiet" disabled={!!busy} onClick={() => { void flush().then(onBack).catch(() => {}); }}><ArrowLeft size={17}/> All content</button>}<div className="content-save" role="status">{saveStatus === 'Saved' ? <Check size={14}/> : saveStatus === 'Saving…' ? <LoaderCircle className="content-spin" size={14}/> : null}{saveStatus}</div></header>
    {!packageMode && <div className="content-draft-heading"><div><span className="content-eyebrow">{CONTENT_BRANDS[draft.brand]} · {draft.kind === 'social-pack' ? 'SOCIAL POSTS' : draft.kind.toUpperCase()}</span><h1>Make it yours.</h1></div><span className={`content-badge ${dirty ? 'draft' : draft.status}`}>{dirty ? 'Needs review' : label(draft.status)}</span></div>}
    ...
    {missingContentImages({...draft,payload:edit.payload}) && <div className="content-notice" role="status"><p>This draft needs its artwork before it can be scheduled or saved to GHL. Your writing is saved.</p><button className="content-button" disabled={readOnly||!!busy} onClick={()=>void action('images')}><Sparkles size={16}/>{busy==='images'?'Generating artwork…':'Generate required images'}</button></div>}
    <div className="content-edit-bar">{!['social-pack','video'].includes(draft.kind)&&<div className="content-segment"><button aria-pressed={!preview} onClick={() => setPreview(false)}>Edit source</button><button aria-pressed={preview} onClick={() => setPreview(true)}>Preview</button></div>}<button className="content-button" disabled={readOnly || !!busy} onClick={() => {setTargetPost(undefined);setRevision('');setReviseOpen(true);}}><Sparkles size={16}/> Ask for a change</button></div>
    {draft.kind==='social-pack'?<ContentSocialStudio brand={draft.brand} posts={edit.payload.posts??[]} disabled={readOnly||!!busy} onChange={posts=>change({payload:{...edit.payload,posts}})} onRevise={index=>{setTargetPost(index);setRevision('');setReviseOpen(true);}} renderImageEditor={(post,index)=><ImageEdit value={post.image} disabled={readOnly||!!busy} required onChange={image=>change({payload:{...edit.payload,posts:edit.payload.posts!.map((p,i)=>i===index?{...p,image}:p)}})}/>}/>:draft.kind==='video'?<ContentVideoStudio draft={draft} disabled={readOnly||!!busy} onRevise={scene=>{setTargetPost(undefined);setRevision(`Change scene ${scene}: `);setReviseOpen(true);}}/>:<div className="content-paper">
      {preview ? <><h2 className="content-preview-title">{edit.title || 'Untitled'}</h2>{draft.kind === 'social-pack' ? edit.payload.posts?.map((post,index) => <article className="content-social-preview" key={index}><span className="content-kind">{CONTENT_CHANNELS[post.platform as ContentChannel] || post.platform}</span><ReactMarkdown remarkPlugins={[remarkGfm]}>{post.text}</ReactMarkdown><ImagePreview value={post.image}/>{post.visual_note && <small>Visual idea: {post.visual_note}</small>}</article>) : <div className="content-prose"><ImagePreview value={draft.kind==='email'?edit.payload.image:edit.payload.featured_image}/><ReactMarkdown remarkPlugins={[remarkGfm]}>{edit.body_markdown}</ReactMarkdown></div>}</> : <>
        <label className="content-field">Title<input className="content-title-input" aria-label="Draft title" readOnly={draft.kind === 'social-pack'||draft.kind==='video'} value={edit.title ?? ''} disabled={readOnly || !!busy} onChange={(e) => change({ title: e.target.value,...(draft.kind==='email'?{payload:{...edit.payload,subject:e.target.value}}:{}) })}/></label>
        ...
        {draft.kind==='email'&&<><label className="content-field">Email subject<input value={String(edit.payload.subject??edit.title??'')} disabled={readOnly||!!busy} onChange={e=>change({title:e.target.value,payload:{...edit.payload,subject:e.target.value}})}/></label><label className="content-field">Preview text<input value={String(edit.payload.preview_text??'')} disabled={readOnly||!!busy} onChange={e=>change({payload:{...edit.payload,preview_text:e.target.value}})}/></label></>}
        {draft.kind === 'email' && <ImageEdit value={edit.payload.image} required disabled={readOnly || !!busy} onChange={(image) => change({ payload: { ...edit.payload, image } })}/>}
    ...
    {draft.publications?.length ? <div className="content-publications">{draft.publications.map((pub,index) => <p key={index}><strong>{CONTENT_CHANNELS[pub.channel as ContentChannel] || pub.channel}</strong> {label(pub.status)} {pub.status!=='saved-draft' && pub.scheduled_at && Number.isFinite(Date.parse(pub.scheduled_at)) && <time dateTime={pub.scheduled_at}>{new Date(pub.scheduled_at).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })} ({Intl.DateTimeFormat().resolvedOptions().timeZone})</time>} {safeUrl(pub.public_url) && <a href={safeUrl(pub.public_url)} target="_blank" rel="noreferrer">View post ↗</a>}{pub.detail && <span>{pub.detail}</span>}</p>)}</div> : null}
    {!packageMode&&<footer className="content-desk-footer"><span>{dirty ? 'Changes will need a fresh approval.' : draft.status === 'approved' ? 'Approved. Ready when you are.' : 'Nothing goes live until you publish.'}</span><div><button className="content-button" disabled={readOnly || !!busy || (draft.status === 'approved' && !dirty)} onClick={() => void action('approve')}><CheckCheck size={17}/> {busy === 'approve' ? 'Approving…' : 'Approve draft'}</button><button className="content-button primary" disabled={dirty || !!busy || missingContentImages(draft) || draft.status !== 'approved'} onClick={() => setPublishOpen(true)}><Send size={16}/> {draft.kind==='email'?'Save draft to GHL':'Publish'}</button></div></footer>}
    {reviseOpen && <ContentModal title={targetPost===undefined?"What would you like to change?":`Change this ${CONTENT_CHANNELS[edit.payload.posts?.[targetPost]?.platform as ContentChannel]??'social'} post`} onClose={busy ? undefined : () => setReviseOpen(false)}><ContentDictation disabled={!!busy} onText={text=>setRevision(old=>old?`${old} ${text}`:text)}/><label className="content-field">Tell your writer<textarea autoFocus rows={4} placeholder="Make it shorter, warmer, more specific…" value={revision} onChange={(e) => setRevision(e.target.value)}/></label><EngineSelect value={engine} onChange={setEngine}/><p className="content-hint">The revision comes back for your review.</p>{error && <p role="alert" className="content-notice error">{error}</p>}<button className="content-button primary" disabled={!!busy || revision.trim().length < 3} onClick={() => void action('revise', { instruction: revision, engine, ...(targetPost===undefined?{}:{targetPost}) })}><Sparkles size={16}/>{busy ? 'Revising…' : 'Revise draft'}</button></ContentModal>}
    {publishOpen && <ContentModal title="Ready to share?" onClose={busy ? undefined : () => setPublishOpen(false)}><p className="content-hint">{draft.kind==='email'?'Save the approved email as a draft in GHL. Confirm the subject, sender and audience there before sending.':'Publish your approved version to the selected destinations.'}</p><fieldset className="content-choices"><legend>Where it will appear</legend>{draft.channels.filter((ch) => ch in CONTENT_CHANNELS).map((channel) => { const connection = connections.find((item) => item.channel === channel); return <label key={channel}><input type="checkbox" checked={selectedChannels.includes(channel)} onChange={(e) => setSelectedChannels((old) => e.target.checked ? [...old,channel] : old.filter((c) => c !== channel))}/>{CONTENT_CHANNELS[channel as ContentChannel]} <small>{connection && ['ready','connected'].includes(connection.status) ? 'Connected' : 'Connection needed'}</small></label>; })}</fieldset>{!available && <p className="content-notice">A selected account still needs connecting. Your approved draft will stay here until it is ready.</p>}{scheduling && <label className="content-field"><span><Clock3 size={14}/> Schedule for later <small>Optional · your local time</small></span><input type="datetime-local" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)}/></label>}{error && <p className="content-notice error" role="alert">{error}</p>}<button className="content-button primary" disabled={!!busy || !available || (!!scheduledAt && (!scheduling || !Number.isFinite(Date.parse(scheduledAt)) || new Date(scheduledAt).getTime() <= Date.now()))} onClick={() => void action('publish', { channels: selectedChannels, ...(scheduledAt ? { scheduledAt: new Date(scheduledAt).toISOString() } : {}) })}><Send size={16}/>{busy ? 'Saving…' : draft.kind==='email' ? 'Save draft to GHL' : scheduledAt ? 'Schedule approved content' : 'Publish approved content'}</button></ContentModal>}
  </section>;
}
